...
clientA.on("new_coupon", (data) => {
  console.log(
    "\n✅ SUCCESS! ClientA received new_coupon event:",
...
async function triggerCoupon() {
  console.log(
    "[Test] Triggering coupon creation via POST /api/admin/coupons...",
  );
  ...
  if (!adminToken) {
    console.log(
      "[Test] No ADMIN_TOKEN env var set. Will test socket emit directly instead.",
    );
    testDirectEmit();
    return;
  }
...
function testDirectEmit() {
  console.log(
    "[Test] Testing via a second client simulating the server-side emit...",
  );
  ...
  console.log(
    "[Test] Check socket_debug.log on the backend to see if join_user was received",
  );
  console.log(
    '[Test] If backend shows "joined user room: user_test123", the fix worked!',
  );

  setTimeout(() => {
    console.log(
      "[Test] Test complete. Check backend socket_debug.log for join_user events",
    );
    cleanup(0);
  }, 2000);
}
