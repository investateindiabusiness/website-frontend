...
import { Mail, Phone, ShieldCheck, ShieldAlert, CheckCircle, XCircle, TrendingUp, Clock, FileWarning, Plus, Trash2, Eye } from 'lucide-react';
...
const AdminInvestors = () => {
  ...
  const filteredInvestors = investors.filter(investor => {
    ...
    if (filter === 'changes_requested') return investor.onboardingStatus === 'form1_changes_requested';
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          {loading ? (
            ...
            <Card key={investorId} className="border border-gray-100 shadow-lg flex flex-col relative overflow-hidden">
              <div className={`h-1 w-full ${investor.onboardingStatus === 'complete' ? 'bg-green-500' : investor.onboardingStatus === 'form1_changes_requested' ? 'bg-orange-500' : 'bg-blue-500'}`} />
                ...
                <div className="flex items-start space-x-6 mb-6">
                  ...
                  <div className="flex-1">
                    <div className="flex items-start justify-between mb-2 gap-2">
                      <h3 className="text-xl font-bold text-gray-900 truncate">{investor.fullName || investor.name || 'Unnamed Investor'}</h3>
                      {renderStatusBadge()}
                    </div>
      ...
      <Dialog open={!!viewInvestorData} onOpenChange={(open) => !open && setViewInvestorData(null)}>
        <DialogContent className="max-w-4xl max-h-[85vh] overflow-y-auto p-0">
          <DialogHeader className="p-6 pb-0 sticky top-0 bg-white z-10 border-b border-gray-100">
            <DialogTitle className="text-2xl font-bold text-gray-900">{viewInvestorData?.fullName || 'Investor Details'}</DialogTitle>
            <DialogDescription>Review the complete submission history for this investor.</DialogDescription>
          </DialogHeader>
            ...
            <div className="p-6 space-y-8 bg-gray-50">
              ...
              <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-y-4 gap-x-8 text-sm">
                  ...
                  <div className="md:col-span-2"><span className="text-gray-500 block mb-1">Address</span><span className="font-medium">{viewInvestorData.address}, {viewInvestorData.city}, {viewInvestorData.state}, {viewInvestorData.zip}, {viewInvestorData.country}</span></div>
                </div>
              </div>

              {/* Dynamic Added Fields Section */}
              ...
              {/* Form 2 Section */}
              {(viewInvestorData.onboardingStatus === 'form2_pending' || viewInvestorData.onboardingStatus === 'complete') && (
                <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-y-6 gap-x-8 text-sm">
                    ...
                    <div><span className="text-gray-500 block mb-1">Preferred Project Type</span><span className="font-medium">{viewInvestorData.preferredProjectType || '-'}</span></div>
              ...
              <div className="bg-white p-4 rounded-xl border border-gray-200 shadow-sm flex flex-wrap gap-3 sticky bottom-4">
                {viewInvestorData.onboardingStatus === 'form1_pending' && (
                  <>
                    ...
                    <Button onClick={() => { setSelectedInvestorId(viewInvestorData.uid); setIsRequestModalOpen(true); }} variant="outline" className="flex-1 border-orange-200 text-orange-600 hover:bg-orange-50">Request Changes</Button>
                  </>
                )}
                {viewInvestorData.onboardingStatus === 'form2_pending' && (
                  <>
                    <Button onClick={() => handleFinalVerification(viewInvestorData.uid, true)} className="flex-1 bg-green-600 hover:bg-green-700 text-white"><CheckCircle className="w-4 h-4 mr-2" /> Verify Account</Button>
                ...
                {viewInvestorData.onboardingStatus === 'form1_changes_requested' && (
                  <div className="w-full text-center text-sm font-medium text-orange-600 bg-orange-50 py-3 rounded-md">Waiting for investor to submit requested changes.</div>
                )}
              </div>
            </div>
          )}
      ...
      <Dialog open={isRequestModalOpen} onOpenChange={setIsRequestModalOpen}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>Specify additional fields the investor needs to provide.</DialogDescription>
          </DialogHeader>
          <div className="py-4 space-y-4">
            {requestedFields.map((field, idx) => (
              <div key={idx} className="flex gap-3 items-end">
                <div className="flex-1">
                  ...
                  <Input value={field.fieldName} onChange={(e) => { const updated = [...requestedFields]; updated[idx].fieldName = e.target.value; setRequestedFields(updated); }} placeholder="e.g. Identity Proof" />
                </div>
                <div className="w-1/3">
            ...
            <Button onClick={submitChangeRequest} className="w-full bg-orange-600 hover:bg-orange-700 text-white mt-6">Send Request to Investor</Button>
          </div>
        </DialogContent>
      </Dialog>
      <Footer />
