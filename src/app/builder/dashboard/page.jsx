            ...
            <div className="grid grid-cols-2 gap-3">
              {[
                ...
                <div key={idx} className="bg-white/10 backdrop-blur border border-white/10 rounded-2xl p-4 flex flex-col justify-between">
                  <div className={`p-2 rounded-xl bg-white/10 ${stat.color} w-fit`}>
                    <stat.icon className="w-4 h-4" />
                  </div>
                  <div className="mt-3">
                    <p className="text-xl font-bold">{isLoading ? '—' : stat.value}</p>
                    <p className="text-[10px] text-blue-200 uppercase tracking-wider opacity-80">{stat.label}</p>
                  </div>
                </div>
              ))}
            </div>
      ...
      <div className="container mx-auto px-4 md:px-6 -mt-10 relative z-10 space-y-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="lg:col-span-2 bg-white rounded-3xl p-6 shadow-sm border border-gray-100">
            <h3 className="font-bold text-gray-900 text-lg flex items-center mb-6">
          ...
          <div className="bg-white rounded-3xl p-6 shadow-sm border border-gray-100 space-y-4">
            <h3 className="font-bold text-gray-900 text-lg mb-2">Project Summary</h3>
            {[
              ...
              <div key={i} className={`${item.bg} rounded-2xl px-4 py-3 flex items-center justify-between`}>
                <div className="flex items-center gap-3">
                  <item.icon className={`w-4 h-4 ${item.color}`} />
                  <span className="text-sm font-semibold text-gray-700">{item.label}</span>
                </div>
                <span className={`text-lg font-extrabold ${item.color}`}>{isLoading ? '—' : item.value}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div>
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-5">
            ...
            <div className="bg-white rounded-2xl border border-gray-200 px-4 py-3 flex items-center gap-2 w-full md:w-80 shadow-sm">
