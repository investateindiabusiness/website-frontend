...
export default function SupportDashboard() {
  ...
  return (
    <main className={`flex-grow pb-12 px-4 md:px-8 max-w-6xl mx-auto w-full ${!isDashboard ? 'mt-[4rem] md:mt-[5rem]' : 'pt-4'}`}>
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
        ...
        <Button 
          onClick={() => router.push(`${rolePrefix}/support/new`)}
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-3xl border border-dashed border-gray-200 p-12 text-center flex flex-col items-center justify-center">
          ...
          {(searchQuery || filterStatus !== 'ALL') ? (
            ...
            <Button onClick={() => router.push(`${rolePrefix}/support/new`)} className="bg-[#0b264f] hover:bg-blue-900 text-white rounded-xl">
              Create Ticket
            </Button>
          )}
        </div>
        ...
        <div className="grid grid-cols-1 gap-4">
          {filteredTickets.map((ticket) => (
            <div 
              key={ticket.id} 
              onClick={() => router.push(`${rolePrefix}/support/${ticket.id}`)}
