  ...
  useEffect(() => {
    ...
    const fetchProjectData = async () => {
      try {
        ...
        if (user && user.uid) {
          const leadStatus = await apiRequest(
            `/api/projects/${id}/lead-status?uid=${user.uid}`,
          );
        ...
        setProject({
          ...
          details: {
            ...
            bankApprovals:
              data.bankApprovals === "Yes"
                ? `Yes (${data.bankApprovalsName})`
                : "No",
            borrowings:
              data.existingBorrowings === "Yes"
                ? `Yes - ${data.existingBorrowingsAmount} (${data.existingBorrowingsPurpose})`
                : "No",
            lockIn: data.lockInPeriod || "N/A",
            buybackGuarantee:
              data.buybackGuarantee === "Yes"
                ? `Yes - ${data.buybackGuaranteeDetails}`
                : "No",
  ...
  const handleRevealContact = () => {
    if (!user) {
      toast({
        ...
        description: "Please log in to view platform contact details.",
      });
    }
  };
  ...
  const handleInquirySubmit = async (e, isDirectCall = false) => {
    ...
    if (hasSubmittedLead)
      return toast({
        ...
        description: "You have already inquired about this project.",
      });
    ...
    try {
      ...
      const payload = {
        ...
        message: isDirectCall
          ? "Investor requested a direct call back."
          : inquiryMessage,
      };
      ...
      toast({
        title: "Success!",
        description:
          "Your request has been sent. An advisor will contact you shortly.",
      });
      setInquiryMessage("");
  ...
  if (loading)
    return (
      <div className="h-screen flex items-center justify-center">
        <Loader2 className="animate-spin text-[#0b264f] h-10 w-10" />
      </div>
    );
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="bg-white p-4 md:p-6 rounded-xl border border-gray-200 shadow-sm mb-6">
              ...
              {/* Image Container */}
              <div
                className="relative h-[250px] md:h-[400px] rounded-xl overflow-hidden group cursor-pointer"
              ...
              >
                ...
                <div className="absolute bottom-4 left-4 flex gap-2">
                  <Button
                    variant="secondary"
                    size="sm"
                    className="bg-white/90 text-black hover:bg-white text-xs h-8 shadow-md"
                  >
                    <Layers className="w-3 h-3 mr-1" /> Photos (
            ...
            <div className="bg-white p-4 md:p-6 rounded-xl shadow-sm border border-gray-200">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-y-6 gap-x-4">
                ...
                <DetailItem
                  label="Total Units"
                  value={project.details.totalUnits}
                ...
                <DetailItem
                  label="Built-up Area"
                  value={project.details.builtUpArea}
                  icon={<Building2 className="w-4 h-4" />}
                />
                <DetailItem
                  label="RERA ID"
                ...
                <DetailItem
                  label="Completion"
                  value={project.details.completionDate}
            ...
            <div className="bg-white p-4 md:p-6 rounded-xl shadow-sm border border-gray-200">
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-2">
                <h2 className="text-lg font-bold text-[#0b264f]">
              ...
              <div className="divide-y divide-gray-100">
                <RowItem
                  label="Project Cost (Approx)"
                  value={project.details.projectCost}
                />
                <RowItem
                  label="Pricing Offered"
                  value={project.details.pricingOffered}
                />
                <RowItem
                  label="Security Against Investment"
                  value={project.details.securityOffered}
                />
                <RowItem
                  label="Govt. Approvals"
                  value={project.details.govtApprovals}
                />
                <RowItem
                  label="Bank Approvals"
                  value={project.details.bankApprovals}
                />
                <RowItem
                  label="Existing Borrowings"
                  value={project.details.borrowings}
                />
                <RowItem
                  label="Lock-in Period"
                  value={project.details.lockIn}
                />
                <RowItem
                  label="Buyback Guarantee"
                  value={project.details.buybackGuarantee}
                />
                <RowItem
                  label="Exit & Resale Framework"
                  value={project.details.exitFramework}
                />
                <RowItem
                  label="Marketing Responsibility"
                  value={project.details.marketingResponsibility}
                />
              </div>
            </div>
            ...
            <div className="bg-white p-4 md:p-6 rounded-xl shadow-sm border border-gray-200">
              ...
              <div className="bg-blue-50 p-5 rounded-lg mb-4 border border-blue-100">
                <p className="text-sm text-blue-900 leading-relaxed whitespace-pre-wrap">
                  {project.details.projectOverview}
                </p>
              </div>
                ...
                <div className="bg-yellow-50 p-4 rounded-lg border border-yellow-100 mt-4">
                  <div className="flex gap-2">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-yellow-700 mt-1 leading-relaxed whitespace-pre-wrap">
                        {project.details.disclosures}
                      </p>
                    </div>
                  </div>
            ...
            {project.approvedDocuments &&
              project.approvedDocuments.length > 0 && (
                <div className="bg-white p-4 md:p-6 rounded-xl shadow-sm border border-gray-200">
                  <h2 className="text-lg font-bold text-gray-800 mb-4 flex items-center">
                    <FileCheck className="w-5 h-5 mr-2 text-[#0b264f]" />{" "}
                    Official Documents
                  </h2>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    {project.approvedDocuments.map((doc, idx) => (
                      <a
                        ...
                        target="_blank"
                        rel="noreferrer"
                        className="flex items-center p-3 bg-gray-50 border border-gray-200 rounded-lg hover:bg-blue-50 hover:border-blue-200 transition-colors group"
                      >
                        <FileCheck className="w-5 h-5 text-gray-400 group-hover:text-blue-600 mr-3" />
                        <span className="text-sm font-medium text-gray-700 group-hover:text-blue-800 flex-1 truncate">
              ...
              <Card className="border-t-4 border-t-[#0b264f] shadow-lg">
                <CardContent className="p-6">
                  ...
                  <p className="text-xs text-gray-500 mb-6">
                    Investate India manages all interactions to ensure
                  ...
                  {!isLoggedIn ? (
                    <div className="text-center">
                      <p className="text-xs text-[#08294F] mb-2 font-medium">
        ...
        <DialogContent className="max-w-5xl bg-white border-none p-0 overflow-hidden text-white flex flex-col justify-center items-center h-[90vh]">
          ...
          <div className="relative w-full h-full flex items-center justify-center p-4">
            {project.images.length > 1 && (
              <button
                onClick={prevImage}
                className="absolute left-4 p-3 bg-black/50 hover:bg-black/80 rounded-full transition-colors z-10"
              >
            ...
            {project.images.length > 1 && (
              <button
                onClick={nextImage}
                className="absolute right-4 p-3 bg-black/50 hover:bg-black/80 rounded-full transition-colors z-10"
              >
...
const DetailItem = ({ label, value, icon, highlight }) => (
  <div className="flex flex-col gap-1">
    ...
    <div
      className={`text-sm md:text-base font-medium truncate ${highlight ? "text-green-600 font-bold" : "text-gray-900"}`}
    >
      {value || "N/A"}
    </div>
  </div>
...
const RowItem = ({ label, value }) => (
  <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start py-3">
    <span className="text-sm text-gray-500 font-medium mb-1 sm:mb-0 w-full sm:w-1/3">
      {label}
    </span>
    <span className="text-sm font-semibold text-gray-900 sm:text-right w-full sm:w-2/3 break-words whitespace-pre-wrap">
      {value || "N/A"}
    </span>
  </div>
);
